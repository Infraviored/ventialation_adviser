...
This document describes the technical architecture of the Ventilation Advisor custom component for Home Assistant.

## Directory Structure

```text
custom_components/ventilation_advisor/
...
│   ├── __init__.py          # Exports VentilationAdvisorDataUpdateCoordinator
...
│   ├── __init__.py          # Exports VentilationAdvisorEntity
...
- `base.py` - Main coordinator class (`VentilationAdvisorDataUpdateCoordinator`)
...
**Key class:** `VentilationAdvisorDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `VentilationAdvisorApiClient`
...
- `VentilationAdvisorConfigFlowHandler` (main flow)
- `VentilationAdvisorOptionsFlow` (options)
...
**Key class:** `VentilationAdvisorEntity` (in `entity/base.py`)
...
2. `VentilationAdvisorEntity` for common functionality
...
1. Create directory: `custom_components/ventilation_advisor/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `VentilationAdvisorEntity`

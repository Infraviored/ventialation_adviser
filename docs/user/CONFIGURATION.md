...
This document describes all configuration options and settings available in the Ventilation Advisor custom integration.
...
### `ventilation_advisor.example_service`
...
```yaml
service: ventilation_advisor.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: ventilation_advisor.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Ventilation Advisor Alert
...
- [GitHub Issues](https://github.com/Infraviored/ventiallation-adviser/issues) - Report problems

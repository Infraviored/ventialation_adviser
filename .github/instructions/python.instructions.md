...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `VentilationAdvisor`
...
- Inherit from both platform entity and `VentilationAdvisorEntity` (order matters)
...
from custom_components.ventilation_advisor.sensor import async_setup_entry
...
**Config entry data:** `entry_data: VentilationAdvisorData = hass.data[DOMAIN][entry.entry_id]`

...
- **Human-Friendly Setup**: Completely overhauled all configuration labels to use natural language (e.g., "Floor Surface" instead of "floor_area").
- **Dynamic Menu Logic**: "Edit Room" and "Delete Room" options now automatically hide if no rooms are configured, preventing broken buttons and technical error messages.
- **Enhanced Translation Loading**: Added `strings.json` to ensure Home Assistant correctly prioritizes user-friendly names over technical keys.

### Fixed

- **Critical Data Entry Crash**: Fixed a conflict where the integration was trying to overwrite a read-only property in the Home Assistant core (AttributeError: config_entry).

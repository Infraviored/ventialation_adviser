...
- **Repository:** Infraviored/ventiallation-adviser

**Key directories:**

- `custom_components/ventilation_advisor/` - Main integration code
...
- Integration logs: `custom_components.ventilation_advisor: debug` in `config/configuration.yaml`
...
- Use the domain `ventilation_advisor` for all DOMAIN references
- Prefix all integration-specific classes with `VentilationAdvisor`
- Use "Ventilation Advisor" as the display title
...
- Inherit from platform base + `VentilationAdvisorEntity`
...
- `tests/` mirrors `custom_components/ventilation_advisor/` structure

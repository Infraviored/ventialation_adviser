...
class VentilationOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        """Manage rooms."""
        return self.async_show_menu(step_id="init", menu_options=["add_room", "remove_room", "system_config"])
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(
                        CONF_STRATEGY,
                        default=self.config_entry.options.get(CONF_STRATEGY, DEFAULT_STRATEGY),
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(CONF_CEILING_HEIGHT, default=DEFAULT_CEILING_HEIGHT): selector.NumberSelector(
                        selector.NumberSelectorConfig(
                            min=1,
                            max=10,
                            step=0.1,
    ...
    async def _update_rooms(self):
        ...
        return self.async_create_entry(title="", data={**self.config_entry.options, CONF_ROOMS: self._rooms})
